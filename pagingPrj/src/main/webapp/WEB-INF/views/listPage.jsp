...
<%
	...
	if( request.getAttribute("p") != null){
	 ...
	 int totRecords =  (Integer )request.getAttribute("totalCnt") ; // 총 레코드 수 
	...
	// int currentPage =1;
	 ...
	 if( currentGrp >1){	%>
	     ...
	     <a href="${path}/list.page?p=<%=index-1%>">[ 이전 ]</a>  
	 ...
	 %>
		  <a href="${path}/list.page?p=<%=index%>">[  <%=index %> ]</a> 
        <%
		  index ++;
	 ...
	 if( index <= totalPage){%>
		   ...
		   <a href="${path}/list.page?p=<%=index%>">[ 다음 ]</a>  
	<% }
	%>
